...
const char SQL_CHAR = 1;
const char SQL_BIT = 1;
...
template <typename T> struct Var_with_AddParams {
    ...
    const int/*SQLSMALLINT*/ targetType;	// тип данных
};
...
const char
    ...
    SERVICE_NAME[]			{ "Все получатели" },	// имя служебного пользователя с индексом 0 (не используется)
    ...
    DATABASE_NAME[]			{ "D:\\chatDB.db3" };	// имя базы данных
